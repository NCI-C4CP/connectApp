...
const submitNewMailingAddress = async (id, addressLine1, addressLine2, city, state, zip, isInternational, addressLine3, country, isPOBox = false, isClearing = false, isValidatedByUSPS = false) => {
  ...
  if (isSuccess) {
    ...
    if (country && country !== '') {
        addressString += translateHTML(`<br><span data-i18n="countries.${Object.keys(cId.countries).find(threeCode => cId.countries[threeCode] === parseInt(country, 10)) || "none"}"></span>`);
    }
                ...
                <div class="col">
                    ...
                    <div class="userProfileBodyFonts" id="profileMailingAddress${id}">
                        ...
                        `
                            ...
                            ${userData[cId.country] ? `<span data-i18n="countries.${Object.keys(cId.countries).find(threeCode => cId.countries[threeCode] === parseInt(userData[cId.country], 10)) || "none"}"></span></br>` : ''}
                            <br>
                        ...
                        <div class="userProfileBodyFonts" id="profileMailingAddress${id}">
                            ...
                            ${!isParticipantDataDestroyed ?
                                `
                                    ...
                                    ${userData[cId.physicalCountry] ? `<br><span data-i18n="countries.${Object.keys(cId.countries).find(threeCode => cId.countries[threeCode] === parseInt(userData[cId.physicalCountry], 10)) || "none"}"></span>` : ''} 
                                ` 
                                : translateText('settings.dataDeleted')
                            }
                            </b>
...
export const renderAlternateAddressData = (id) => {
    ...
    if (hasAddressContent) {
        address = `
            ...
            ${userData[cId.altCountry] ? `<span data-i18n="countries.${Object.keys(cId.countries).find(threeCode => cId.countries[threeCode] === parseInt(userData[cId.altCountry], 10)) || "none"}"></span><br>` : ''} 
            <br>
